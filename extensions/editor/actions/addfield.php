...
if ( $content->load ( $_REQUEST[ 'cid' ] ) )
{
	if ( $GLOBALS[ 'Session' ]->AdminUser->checkPermission ( $content, 'write', 'admin' ) )
	{
		...
		list ( $max1, ) = $db->fetchRow ( 'SELECT MAX(SortOrder) FROM ContentDataSmall WHERE ContentID=' . $content->ID . ' AND ContentTable="ContentElement"' );
		list ( $max2, ) = $db->fetchRow ( 'SELECT MAX(SortOrder) FROM ContentDataBig WHERE ContentID=' . $content->ID . ' AND ContentTable="ContentElement"' );
		$max = max ( $max1, $max2 );
