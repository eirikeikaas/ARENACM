...
arenaGallery.prototype.SetGalleryImage = function ( pelement, num )
{
	...
	{
		if (  gc.imgs[num].description.indexOf ( 'http://' ) >= 0 )
		{
			...
			div.onclick = function (){ window.open( 'http://'+test[1], '', '' ); }
	...
	gc.intv = setInterval ( 'document.arenaGalleries[' + this.index + '].FadeGalleryImageIn(\''+pelement.id+'\')', 5 );
}
arenaGallery.prototype.FadeGalleryImageIn = function ( el )
{
	...
	var gc = this.el.gc;
	gc.img2.op = ( ( new Date () ).getTime() - this._currentTime ) / this.gallerySpeed;
	var alpha = gc.img2.op > 1 ? 1 : gc.img2.op;
	if ( this.prevAlpha != alpha )
	{
		...
		switch ( this.galleryShowStyle )
		{
			case 'showstyle_scrollx':
				...
				var po = Math.floor ( this.galleryWidth * power );
				gc.img1.style.left = -po + 'px';
				gc.img2.style.left = ( this.galleryWidth - po ) + 'px';
				break;
			case 'showstyle_scrolly':
				this.setOp ( gc.img2, alpha );
				var po = Math.floor ( this.galleryHeight * power );
				gc.img1.style.top = -po + 'px';
				gc.img2.style.top = ( this.galleryHeight - po ) + 'px';
				break;
			default:
				this.setOp ( gc.img2, alpha );
				break;
...
function showImage ( fn )
{
	...
	if ( !cachedImages[fn] )
	{
		...
		theImage.onload = function ()
		{
			...
			box.style.top = 0-(20+Math.floor ( (this.height/2) )) + 'px';
			box.style.left = 0-(20+Math.floor ( (this.width/2) )) + 'px';
