...
arenaGallery.prototype.init = function ( pelement )
{
	...
	if ( this.galleryAnimated )
	{
		...
		this.galInterval = setInterval ( 'document.arenaGalleries[' + this.index + '].GalNextButton ( )', this.galleryPause * 1000 );
	}
}
...
arenaGallery.prototype.SetGalleryImage = function ( pelement, num )
{
	...
	gc.img2.op = 0;
	gc.img2.innerHTML = '<div class="Image" style="background:url(\'' + gc.imgs[num].src + '\') no-repeat top left; width:' + this.galleryWidth + 'px; height: ' + ( this.galleryHeight ) + 'px"></div>';
	if ( gc.imgs[num].description && gc.imgs[num].description.length )
	{
		if (  gc.imgs[num].description.substr ( 0, 7 ) == 'http://' )
		{
			var div = gc.img2.getElementsByTagName ( 'div' )[0];
			if( navigator.userAgent.indexOf ( 'MSIE' ) >= 0 )
				div.style.cursor = 'hand';
			else div.style.cursor = 'pointer';
			div.onclick = function (){ window.open( gc.imgs[num].description, '', '' ); }
		}
		else
		{
	...
	gc.intv = setInterval ( 'document.arenaGalleries[' + this.index + '].FadeGalleryImageIn(\''+pelement.id+'\')', 25 );
}
arenaGallery.prototype.FadeGalleryImageIn = function ( el )
{
	el = document.getElementById ( el );
	var gc = el.gc;
	gc.img2.op = (this.getTime ()-this._currentTime) / this.gallerySpeed;
	var alpha = gc.img2.op;
	if ( alpha > 1 )
		alpha = 1;
	var power = Math.sin((alpha-0.5)*Math.PI)*0.5+0.5;
	switch ( this.galleryShowStyle )
	{
		case 'showstyle_scrollx':
			this.setOp ( gc.img2, alpha );
			gc.img1.style.left = ( 0 - Math.floor ( this.galleryWidth*(power) ) ) + 'px';
			gc.img2.style.left = ( this.galleryWidth - Math.floor ( this.galleryWidth*(power) ) ) + 'px';
			break;
		case 'showstyle_scrolly':
			this.setOp ( gc.img2, alpha );
			gc.img1.style.top = ( 0 - Math.floor ( this.galleryHeight*(power) ) ) + 'px';
			gc.img2.style.top = ( this.galleryHeight - Math.floor ( this.galleryHeight*(power) ) ) + 'px';
			break;
		default:
			this.setOp ( gc.img2, alpha );
			break;
...
arenaGallery.prototype.GalNextButton = function ( )
{
	...
	{
		clearInterval ( this.galInterval );
		this.galInterval = setInterval ( 'document.arenaGalleries[' + this.index + '].GalNextButton ( )', 100 );
		return;
	}
...
function showImage ( fn )
{
	if ( !imageview )
	{
		...
		img.onclick = function ()
		{
			...
			imagebg.style.opacity = 0;
			img.style.border = '0px solid white';
			setTimeout ( 'imagebg.style.display = "none";', 500 );
		}
		
		imageview.appendChild ( img );
		base.appendChild ( imagebg );
		base.appendChild ( imageview );
		document.getElementById ( 'Empty__' ).appendChild ( base );
	}
