...
arenaGallery.prototype.SetGalleryImage = function ( pelement, num )
{
	...
	if ( gc.imgs[num].description && gc.imgs[num].description.length )
	{
		if (  gc.imgs[num].description.indexOf ( 'http://' ) >= 0 )
		{
			...
			if ( ( 'http://' + test[1] ).indexOf ( document.getElementsByTagName ( 'base' )[0].href ) >= 0 )
	...
	gc.intv = setInterval ( 'document.arenaGalleries[' + this.index + '].FadeGalleryImageIn(\''+pelement.id+'\')', pad ? 100 : 25 );
}
