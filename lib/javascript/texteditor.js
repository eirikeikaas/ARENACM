...
texteditor.addControl = function ( area )
{
	...
	obj.correctHTML = function ( string )
	{
		...
		var illegals = [ 
			...
			'-mso', 'background', 'vertical-align', 'list-style', ''
		];
		var matches;
		while ( matches = string.match ( /style\=\"([^\>]*?)\"/i ) )
		{
			var styles = matches[1].split ( ';' );
			var newstyles = new Array ();
			for ( var a = 0; a < styles.length; a++ )
			{
				styles[a] = styles[a].toLowerCase ();
				styles[a] = styles[a].split ( /\s/ ).join ( '' );
				var style = styles[a].split ( ':' );
				var found = false;
				for ( var b = 0; b < illegals.length; b++ )
				{
					if ( style[0].indexOf ( illegals[b] ) >= 0 )
					{
						found = true;
						break;
					}
				}
				if ( !found ) newstyles.push ( style[0] + ': ' + style[1] );
			}
			string = string.replace ( matches[0], 's_arena=\"' + newstyles.join ( '; ' ) + '"' );
		}
		string = string.split ( 's_arena' ).join ( 'style' );
		...
		string = string.split ( '<br></p>' ).join ( '</p>' );
		string = string.split ( '<br/></p>' ).join ( '</p>' );
		...
		string = string.split ( /mso\-[^:]*?\:[^;"]*/i ).join ( '' );
		string = string.split ( '="; ' ).join ( '="' );
		// Remove empty styles
		...
		string = string.split ( /\<p\>\<\/p\>/i ).join ( '<br/>' );
		string = string.split ( /\<p\>\<br[^>]*?\>\<\/p\>/i ). join ( '<br/>' );
	...
	obj.insertHTML = function ( html )
	{
		if ( isIE )
		{
			...
			// TODO: Use clickX to find the element that is closest to click x!
			if ( whole == endpart )
			{
		...
		addEventTo ( doc, 'keyup', function ( event )
			{
				...
				switch ( key )
				{
					case 86: 
						...
						{
							// Little cleaning
							var cnt = obj.correctHTML ( obj.getDocument ().body.innerHTML );
							obj.getDocument ().body.innerHTML = cnt;
						}
						break;
				}
				// Remember changes
