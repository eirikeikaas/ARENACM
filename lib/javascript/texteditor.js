...
texteditor.addControl = function ( area )
{
	...
	obj.correctHTML = function ( string )
	{
		...
		while ( matches = string.match ( /(\<.*?)style\=\"([^\>]*?)\"/i ) )
		{
			...
			for ( var a = 0; a < styles.length; a++ )
			{
				...
				{
					if ( style[0].length <= 1 || style[0].indexOf ( illegals[b] ) >= 0 )
					{
						found = true;
						break;
					}
			...
			string = string.replace ( matches[0], matches[1]+'s_arena=\"' + newstyles.join ( '; ' ) + '"' );
		}
	...
	obj.setClass = function ( val )
	{
		...
		if ( nod && nod != this.getDocument().body && nod.nodeName.toLowerCase() == 'span' )
		{
			nod.className = val;
		}
		else
	...
	obj.checkSelection = function ( )
	{
		...
		for ( var a = 0; a < childNodes.length; a++ )
		{
			...
			this.activateFormattingButtonOn ( cn );
	...
	obj.activateFormattingButtonOn = function ( node )
	{
				...
				switch ( key )
				{
					case 86: 
						...
						if ( event.ctrlKey )
						{
							...
							if ( cnt.length != obj.getDocument().body.innerHTML.length )
