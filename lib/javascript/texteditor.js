...
//		{ 'type' : 'button', 'classname' : 'bCleanHTML', 'action' : 'cleanHTML ( )', 'title' : 'Rensk utvalg', 'icon' : 'wrench_orange.png' },
	...
	{ 
		if ( obj.activateEvents ) 
			obj.activateEvents ( obj ); 
		this.contentDocument.designMode = 'on';
		if ( obj.stylesheetSrc && !obj.loadedDone )
		{
			var t = new Date ();
			var link = obj.contentDocument.createElement ( 'link' );
			link.rel = 'stylesheet';
			link.href = obj.obj.stylesheetSrc + '?rand=' + (t.getTime()+(Math.random()*10000000)+(Math.random()*10000000));
			obj.contentDocument.getElementsByTagName ( 'head' )[ 0 ].appendChild ( link );
		}
		obj.loadedDone = true;
	}
