...
texteditor.addControl = function ( area )
{
	...
	obj.GenerateToolbar ( Array ( 
		...
		{ 'type' : 'button', 'classname' : 'bAlignRight', 'action' : 'alignRight ( )', 'title' : 'HÃ¸yrejustert', 'icon' : 'text_align_right.png' },
		...
		{ 'type' : 'button', 'classname' : 'bRedo', 'action' : 'redo ( )', 'title' : 'GjÃ¸r om igjen', 'icon' : 'arrow_redo.png' }
	) );
	...
	{	
		...
		cnt = cnt.split ( /\<[/]*u[^>]*?\>/ ).join ( '' );
		cnt = cnt.split ( /\<[/]*i[^>]*?\>/ ).join ( '' );
		cnt = cnt.split ( /\<[/]*em[^>]*?\>/ ).join ( '' );
		cnt = cnt.split ( /\<[/]*b[^>]*?\>/ ).join ( '' );
		cnt = cnt.split ( /\<[/]*h[^>]*?\>/ ).join ( '' );
