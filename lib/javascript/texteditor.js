...
texteditor.addControl = function ( area )
{
	...
	obj.insertHTML = function ( html )
	{
		if ( isIE )
		{
			...
			var r;
			if ( r = this.getDocument ().selection.createRange () )
			{
				if ( r.text.length <= 0 )
				{
					r.expand ( 'character' );
				...
				if ( r.text.length > 0 )
				{
					var rand = '!special'+(Math.random()%999)+(Math.random()%999)+'!';
					r.text = rand;
					this.getDocument().body.innerHTML = this.getDocument().body.innerHTML.split(rand).join(html);
					return true;
				}
			}
