...
texteditor.addControl = function ( area )
{
	...
	obj.GenerateToolbar ( Array ( 
		{ 'type' : 'select', 'classname' : 'bFormatting', 'action' : 'setFormatting ( this.value )', 
			'list' : '-:Velg formattering;none:Standard;p:Paragraf;h1:Overskrift 1;h2:Overskrift 2;h3:Overskrift 3;h4:Overskrift 4;pre:Preformattert kode;' },
		...
		adv ? { 'type' : 'button', 'classname' : 'bYouTube', 'action' : 'insertYoutube ( )', 'title' : 'Sett inn Youtube link', 'icon' : 'youtube_add.png' } : {},
		...
		{ 'type' : 'button', 'classname' : 'bAlignJustify', 'action' : 'alignJustify ( )', 'title' : 'Blokkjustert', 'icon' : 'text_align_justify.png' },
		adv ? { 'type' : 'spacer' } : {},
		{ 'type' : 'button', 'classname' : 'bIndent', 'action' : 'indent ( )', 'title' : 'Innrykk', 'icon' : 'text_indent.png' },
		{ 'type' : 'button', 'classname' : 'bOutdent', 'action' : 'outdent ( )', 'title' : 'Fjern innrykk', 'icon' : 'text_indent_remove.png' },
		...
		adv ? { 'type' : 'spacer' } : {},
//		{ 'type' : 'button', 'classname' : 'bCleanHTML', 'action' : 'cleanHTML ( )', 'title' : 'Rensk utvalg', 'icon' : 'wrench_orange.png' },
		adv ? { 'type' : 'button', 'classname' : 'bCleanAllHTML', 'action' : 'cleanAllHTML ( )', 'title' : 'Rensk hele feltet', 'icon' : 'page_white_wrench.png' } : {},
		this.mode == 'admin' ? { 'type' : 'button', 'classname' : 'bRemoveAllHTML', 'action' : 'removeAllHTML ()', 'title' : 'Ta vekk formattering', 'icon' : 'page_white_code_red.png' } : {},
		adv ? { 'type' : 'spacer' } : {},
		adv ? { 'type' : 'button', 'classname' : 'bScript', 'action' : 'toggleSource ( );", 100 ); }', 'title' : 'Se kildekode', 'icon' : 'html.png' } : {},
		adv ? { 'type' : 'spacer' } : {},
		adv ? { 'type' : 'button', 'classname' : 'bUndo', 'action' : 'undo ( )', 'title' : 'Angre', 'icon' : 'arrow_undo.png' } : {},
		adv ? { 'type' : 'button', 'classname' : 'bRedo', 'action' : 'redo ( )', 'title' : 'Gjør om igjen', 'icon' : 'arrow_redo.png' } : {}
	) );
	...
	if ( navigator.userAgent.indexOf ( 'MSIE' ) >= 0 )
	{
		addEventTo ( obj.iframe, 'onload', function ( ){ 
			...
			if ( obj.stylesheetSrc )
			{
				...
				var link = obj.getDocument ( ).createElement ( 'link' );
				link.rel = 'stylesheet';
				link.href = path + obj.stylesheetSrc;
				var def = obj.getDocument ( ).createElement ( 'link' );
				def.rel = 'stylesheet';
				def.href = path + 'lib/templates/texteditor.css';
				obj.getDocument ( ).getElementsByTagName ( 'head' )[ 0 ].appendChild ( def );
				obj.getDocument ( ).getElementsByTagName ( 'head' )[ 0 ].appendChild ( link );
				obj.getDocument ( ).documentElement.style.borderStyle = 'none';
			}
		} );
	}
	...
	{ 
		...
		if ( this.obj.stylesheetSrc )
		{
			var t = new Date ();
			var link = this.contentDocument.createElement ( 'link' );
			link.rel = 'stylesheet';
			link.href = this.obj.stylesheetSrc + '?rand=' + (t.getTime()+(Math.random()*10000000)+(Math.random()*10000000));
			this.contentDocument.getElementsByTagName ( 'head' )[ 0 ].appendChild ( link );
		}
	}
	...
	obj.setClass = function ( val )
	{
		...
		if ( nod && nod != this.getDocument().body )
		{
			nod.className = val;
		}
		else
	...
	obj.createLink = function ( )
	{
		...
		else if ( node && !node.href && node.parentNode && node.parentNode.href )
		{
			node = node.parentNode;
		}
		...
		else
		{
			...
			if ( !node )
			{
				...
				{
					// Try to create a temporary textnode incase we undo and end 
					// up with an empty link when cancelling insert link popup
					var nod = this.createTempNode ();
					this.linkNode = nod;
					if ( nod.parentNode )
					{
						var textnode = this.getDocument().createTextNode ( nod.innerHTML );
	...
	obj.destroyLink = function ( )
	{
		...
		{
			var t = this.getDocument().createTextNode ( nod.innerHTML );
	...
	obj.insertYoutube = function ()
	{
		...
		{
			var jax = new bajax ();
			jax.openUrl ( 'admin.php?plugin=texteditor&pluginaction=getyoutube&lnk='+lnk, 'get', true );
			jax.o = this;
			jax.onload = function ()
			{
				var result = this.getResponseText () + '';
				result = result.split ( '<!SEPARATE!>' );
				if ( result[0].indexOf ( '.swf' ) > 0 && result[1].length )
				{
					var d = new Date ();
					var str = '' +
					'<span arenatype="movie" style="width: 480px; height: 270px; display: block; border: 2px dotted #aaa; background: #ccc url(admin/gfx/arenaicons/page_flash_64.png) no-repeat center center" id="flash_' + d.getTime() + '" data="http://' + result[0] + '" width="480" height="270" wmode="transparent" type="application/x-shockwave-flash" flashvars="' + result[1] + '">' +
					'	<param name=\"width\" value=\"480\"></param>' +
					'	<param name=\"height\" value=\"270\"></param>' +
					'	<param name=\"movie\" value=\"http://' + result[0] + '\"></param>' +
					'	<param name=\"wmode\" value=\"transparent\"></param>' +
					'</span>&nbsp;'
					this.o.insertHTML ( str );
				}
	...
	// Remove all HTML if we delete all content except empty tags
	obj.checkEmptyHTML = function ()
	{
		var b = this.getDocument ().body;
		var cnt = b.innerHTML;
		cnt = cnt.split ( /\<img[^>]*?\>/i ).join ( 'IMAGE' ); // Make sure these
		cnt = cnt.split ( /\<input[^>]*?\>/i ).join ( 'INPUT' ); // tags aren't 
		cnt = cnt.split ( /\<hr[^>]*?\>/i ).join ( 'HR' ); // treated as whitespace
		cnt = cnt.split ( /\<[^>]*?\>/i ).join ( '' ).split ( ' ' ).join ( '' ).split ( /\s\S/i ).join ( '' );
		cnt = cnt.split ( /\&[a-zA-Z]{1,4}\;/ ).join ( '' );
		if ( !cnt.length )
		{
			// Remove content and restore selection
			var r;
			if ( window.getSelection ) 
				r = this.getSelection().getRangeAt( 0 );
			else if( document.selection )
				r = this.getSelection().createRange();  
			b.innerHTML = '';
			if ( r.select ) r.select ();
			else this.getSelection().addRange ( r );	
	...
	obj.insertHTML = function ( html )
	{
		...
		{
			// Find selected text
			var b = this.getDocument ( ).selection;
			r = b.createRange ( ).duplicate ( );
			r.moveStart ( 'sentence', -1000000 );
			r.moveEnd ( 'sentence', 1000000 );
			var whole = r.text.length;
			r = b.createRange ( ).duplicate ( );
			r.moveEnd ( 'sentence', 1000000 );
			var endpart = r.text.length;
			var firstpart = whole - endpart;
			
			// (scary music) calculate approximately where to place html
			// here, insert by cursor
			if ( whole == endpart )
			{
				r = this.getDocument ( ).body.createTextRange ( ).duplicate ( );
				...
				{
					var midy = Math.abs ( ( getElementTop ( eles[ a ] ) + ( getElementHeight ( eles[ a ] ) / 2 ) ) - this.clickY );
					if ( midy < miny )
					{
						mino = eles[ a ];
						miny = midy;
				...
				if ( !mino && this._lastClickedItem && this._lastClickedItem.nodeName != 'BODY' )
				{
					var d = this.getDocument().createElement ( 'div' );
					d.innerHTML = html;
					this._lastClickedItem.parentNode.replaceChild ( d.firstChild, this._lastClickedItem );
					delete d;
				}
				else
				{
					// Create our insertable textnode
					var d = this.getDocument ( ).createElement ( '#text' );
					d.innerHTML = html;
					// Tricksy! Could go wrong!
					if ( !mino )
					{
						if ( this.getDocument ( ).body.childNodes.length ) 
							mino = this.getDocument ( ).body.childNodes[0];
						else mino = this.getDocument ( ).body;
					}
					if ( mino.parentNode && mino.nodeName.toLowerCase ( ) != 'body' )
					{
						var pnode = mino.parentNode;
						pnode.insertBefore ( d, mino );
						pnode.removeChild ( mino );
						pnode.insertBefore ( mino, d );
					}
					// If we couldn't get a node from the body or if mino is body
					else if ( ( mino && mino.nodeName.toLowerCase ( ) == 'body' ) || !mino )
					{
						this.getDocument ( ).body.innerHTML = html + this.getDocument ( ).body.innerHTML;
					}
					
					// Remove textnode crap
					var ht = this.getContent ( );
					ht = ht + '';
					ht = ht.split ( '<#text>' ).join ( '' ).split ( '</#text>' ).join ( '' );
					if ( pnode && ht && pnode.innerHTML ) 
					{
						try
						{
							pnode.innerHTML = ht;
	...
	obj.cleanHTML = function ( )
	{
		// Remove formatting
		this.deepCleanHTML ( );
		this.getDocument().execCommand ( 'removeformat', false, false );
	}
	...
	{	
		...
		// Remove styles
		var mat;
		while ( mat = cnt.match ( /(style=["]{0,1}[^>]*)/ ) )
			cnt = cnt.split ( mat[ 0 ] ).join ( '' ); 
		// Remove classes
		while ( mat = cnt.match ( /(class=["]{0,1}[^>]*)/ ) )
			cnt = cnt.split ( mat[ 0 ] ).join ( '' ); 
		// Remove width
		while ( mat = cnt.match ( /(width=["]{0,1}[^>]*)/ ) )
			cnt = cnt.split ( mat[ 0 ] ).join ( '' ); 
		// Remove height
		while ( mat = cnt.match ( /(height=["]{0,1}[^>]*)/ ) )
			cnt = cnt.split ( mat[ 0 ] ).join ( '' ); 
		// Remove border
		while ( mat = cnt.match ( /(border=["]{0,1}[^>]*)/ ) )
			cnt = cnt.split ( mat[ 0 ] ).join ( '' ); 
	...
	obj.setFormatting = function ( ftype )
	{
			...
			switch ( ftype )
			{
				...
				case 'P':
					...
					if ( n.tagName && !sel && n.parentNode )
					{
						var nod = n.parentNode;
						var ele = this.getDocument().createElement ( ftype );
					...
					else
					{
						...
						var as = this.createTempNode ();
						if ( sel.length > 0 && as.parentNode )
						{	
							// Create temp string with everything in the parentnode except the selected
							var pnodeWithoutTmpNode = as.parentNode.innerHTML + '';
							pnodeWithoutTmpNode = pnodeWithoutTmpNode.split ( as.getAttribute ( 'href' ) ).join ( '' );
							pnodeWithoutTmpNode = pnodeWithoutTmpNode.split ( /\<a href\=\"\"[^>]*?\>.*?\<\/a>/i ).join ( '' );
							...
							{
								var nod = this.getDocument().createElement ( ftype );
								nod.innerHTML = as.innerHTML;
								as.parentNode.parentNode.replaceChild ( nod, as.parentNode );
							}
							...
							else
							{
								var nod = this.getDocument().createElement ( ftype );
								nod.innerHTML = as.innerHTML;
								as.parentNode.replaceChild ( nod, as );
							}	
						}
	...
	{
		var linkc = 'special_link' + ( Math.random ( ) * 9999 ) + ( Math.random ( ) * 9999 );
	...
	obj.viewFormatting = function ( frm )
	{
		var select = this.toolbarButtons.bFormatting.getElementsByTagName ( 'select' );
		select = select[ 0 ];
		select.options[ 0 ].selected = "selected";
		for ( var a = 1; a < select.options.length; a++ )
		{
	...
	obj.removeControl = function ( options )
	{
		...
		if ( this.iframe ) this.iframe.contentEditable = false;
		...
		for ( var a = 0; a < childNodes.length; a++ )
		{
			...
			switch ( nn )
			{
				case '#text':
					...
					break;
				case 'span':
					var fw = getStyle ( cn, 'font-weight' );
					if ( typeof ( fw ) == 'number' && fw >= 700 )
						this.buttonOn ( this.toolbarButtons.bBold );
					else if ( typeof ( fw ) == 'string' && fw.indexOf ( 'bold' ) >= 0 )
						this.buttonOn ( this.toolbarButtons.bBold );
					if ( getStyle ( cn, 'text-decoration' ).indexOf ( 'underline' ) >= 0 )
						this.buttonOn ( this.toolbarButtons.bUnderline );
					if ( getStyle ( cn, 'font-style' ).indexOf ( 'italic' ) >= 0 )
						this.buttonOn ( this.toolbarButtons.bItalic );
					if ( getStyle ( cn, 'font-style' ).indexOf ( 'opaque' ) >= 0 )
						this.buttonOn ( this.toolbarButtons.bItalic );
					break;
				case 'strong':
				case 'b':
	...
	obj.showProperties = function ( parentnode )
	{
		...
		initModalDialogue ( 'elementproperties', 400, 480, 'admin.php?pluginaction=properties&plugin=texteditor' );
	}
	...
	obj.showPopupMenu = function ( )
	{
		...
		{
			document._editorpopupmenumovefunc;
	...
	obj.activateEvents = function ( fr )
	{
		...
		addEventTo ( doc, 'keyup', function ( event )
			{
				...
				switch ( key )
				{
					...
					case 8:
						obj.checkEmptyHTML ();
						break;
					case 86: if ( event.ctrlKey ) obj.cleanBadHTML (); break;
					default:
						break;
				}
				// Remember changes
		...
		addEventTo ( doc, 'mousedown', function ( e )
			{
				obj.resizeToFit ();
				parent.winele__.onmousedown ( parent.window.Event );
			}
		);
		addEventTo ( doc, 'mouseup', function ( e )
			{
				...
				parent.winele__.onmouseup ( parent.window.Event );
			}
		);
		addEventTo ( doc, 'mousemove', function ( event )
			{
				if ( !event ) event = window.event;
				parent.winele__.onmousemove ( parent.window.Event );
				var posx = 0;
				var posy = 0;
				if ( !event ) 
				{
					if ( fr.document )
						event = fr.document.event;
					else event = fr.contentWindow.event;
				}
	...
	var jax = new bajax ( );
	jax.openUrl ( 'lib/templates/texteditor.html', 'get', true );
	jax.obj = obj;
	jax.onload = function ( )
	{
	...
	document.getElementById ( 'LinkEditDone' ).onclick = function ( )
	{
		...
		{	
			...
			if ( ed.linkMode == 'bookmark' )
			{
				...
				node = ed.getSelectedNode ( );
				ed.linkNode = node;
				
				if ( !node ) node = ed.getSelectedNode (); 
			}
