	...
	document.getElementById ( 'LinkEditDone' ).onclick = function ( )
	{
		...
		{	
			...
			if ( ed.linkMode == 'bookmark' )
			{
				...
				{
					alert ( 'foo' );
					ed.getDocument ().body.focus ();
					var range = ed.getRange ();
					var linkspecial = "special"+(Math.random()*100000);
					node = false;
					range.pasteHTML ( '<a href="'+linkspecial+'">' + range.htmlText + '</a>' );
					var as = ed.getDocument().getElementsByTagName ( 'a' );
					for ( var a = 0; a < as.length; a++ )
					{
						if ( as[a].href.indexOf ( linkspecial ) >= 0 )
						{
							node = as[a];
							ed.linkNode = false;
						}
