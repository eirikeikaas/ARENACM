...
var ModalDialogue = function ( )
{
	...
	{
		this.Foreground.innerHTML = '<div id="ModalForeground" class="SubContainer" style="display: block; overflow: auto; height: ' + ( this.Height - 4 - 14 ) + 'px">' + this._content + '</div>';
	...
	this._fadein = function ( varfunc )
	{
		...
		var diff = ( this._phase - this._target ) / this._speed;
		var timeout = '';
        ...
        var dOffX = 1 + ( getVerticalScrollbarWidth ( ) / 2 );
		var dOffY = 1;
		var dWidth = ( this.Width ) / this._target * this._phase;
		var dHeight = ( this.Height ) / this._target * this._phase;
		
		if ( this._phase == this._target )
		{
		...
		{
			setOpacity ( this.Background, this._phase / this._target * 0.6 );
		}

		this.DiaBox.style.top = ( Math.round ( BgWidth / 2 - ( dHeight / 2 ) ) + dOffY + ( isIE6 ? getScrollTop ( ) : 0 ) ) + 'px';
		this.DiaBox.style.left = ( Math.round ( BgHeight / 2 - ( dWidth / 2 ) ) + dOffX ) + 'px';
		this.DiaBox.style.width = Math.round ( dWidth ) + 'px';
		this.DiaBox.style.height = Math.round ( dHeight ) + 'px';
		
		if ( timeout.length )
			setTimeout ( timeout, timeout.indexOf ( 'fadein' ) >= 0 ? 50 : 1 );
	}
	...
	this.refresh = function ( )
	{
		...
		var dOffX = 1 + ( getVerticalScrollbarWidth ( ) / 2 );
		var dOffY = 1;
		var dWidth = ( this.Width + BorderWidth2 - 2 ) / this._target * this._phase;
		var dHeight = ( this.Height + BorderWidth2 - 2 ) / this._target * this._phase;
		...
		this.Foreground.firstChild.style.height = ( this.Height - 4 - 14 ) + 'px';
	}
	...
	this._fadeout = function ( varfunc )
	{
		...
		if ( this._fademode != '' && this._fademode != 'out' )
			return;
		this._fademode = 'out';
		var diff = this._phase / this._speed;
		var timeout = '';
		...
		if ( timeout.length )
			setTimeout ( timeout, timeout.indexOf ( 'fadeout' ) >= 0 ? 50 : 1 );
	}
	
	this._setDiaBox = function ( )
	{
		var dWidth = this.Width / this._target * this._phase;
		var dHeight = this.Height / this._target * this._phase;
		this.DiaBox.style.top = Math.round ( getElementHeight ( this.Background ) / 2 - ( dHeight / 2 ) + ( isIE6 ? getScrollTop ( ) : 0 ) ) + 'px';
		this.DiaBox.style.left = Math.round ( getElementWidth ( this.Background ) / 2 - ( dWidth / 2 ) ) + 'px';
		this.DiaBox.style.width = Math.round ( dWidth ) + 'px';
		this.DiaBox.style.height = Math.round ( dHeight ) + 'px';
	}
}
...
function removeModalDialogue ( varname )
{
	...
	for ( var a = 0; a < document.modaldialogues.length; a++ )
	{
		if ( document.modaldialogues[ a ].Name == varname )
		{
			...
			document.modaldialogues[ a ]._phase = 10;
			document.modaldialogues[ a ]._speed = 1;
	...
	document.repljax.onload = function ( )
	{
		for ( var a = 0; a < document.modaldialogues.length; a++ )
		{
			if ( document.modaldialogues[ a ].Name == varname )
			{
				...
				document.modaldialogues[ a ]._setDiaBox ();
			}
		}	
		if ( this.queuefunc )
			this.queuefunc ( );
