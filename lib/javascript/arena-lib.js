...
var ModalDialogue = function ( )
{
	...
	{
		if ( document.getElementById ( 'ModalForeground' ).intr )
		{
			clearInterval ( document.getElementById ( 'ModalForeground' ).intr );
		}
	...
	this._renderForeground = function ( )
	{
		...
		document.getElementById ( 'ModalForeground' ).intr = 
			setInterval ( "document.getElementById ( 'ModalForeground' ).modal.refresh()", 250 );
	}
		...
		else if ( this.mode == 'blest' )
		{
			...
			if ( ed )
			{
				if ( ed._lastClickedItem && ed._lastClickedItem.nodeName.toLowerCase () == 'img' )
				{
					...
					if ( isIE ) oldStyleS = oldStyleS.cssText.toLowerCase();
					var oldStyle = [];
					// retrieve style
					if ( oldStyleS )
					{
