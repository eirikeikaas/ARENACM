...
var ModalDialogue = function ( )
{
	...
	this._renderForeground = function ( )
	{
		...
		this.Foreground.onmousedown = function ()
		{
			...
			setTimeout ( 'document._tempForeground.refresh(); document._tempForeground = false;', 250 );
		}
		...
		setTimeout ( 'document.getElementById(\'ModalForeground\').modal.refresh()', 100 );
	}
	...
	{
		this.Height = this.Foreground.offsetHeight;
		this.setHeight ( this.Height );
		
		var BgWidth = getElementHeight ( this.Background );
		var BgHeight = getElementWidth ( this.Background );
		var BorderWidth = 12 * this._phase / 100;
		var BorderHeight = 6 * this._phase / 100;
		var dOffX = 3 - ( getVerticalScrollbarWidth ( ) * 0.5 );
		var dOffY = 3;
		var dWidth = Math.round ( ( this.Width ) / this._target * this._phase );
		var dHeight = Math.round ( ( this.Height ) / this._target * this._phase );
...
var EditorAbstraction = function ( mode, options )
{
	...
	this.insertImage = function ( image, edid )
	{
		...
		else if ( this.mode == 'blest' )
		{
			...
			if ( ed )
			{
				if ( ed._lastClickedItem.nodeName.toLowerCase () == 'img' )
				{
					...
					var width = ed._lastClickedItem.style.width;
					var height = ed._lastClickedItem.style.height;
					// Assign new attributes (except width and height, which we keep)
					for ( var z = 0; z < attrs.length; z++ )
					{
						if ( attrs[z][0].toLowerCase() == 'style' )
						{
							...
							for ( var b = 0; b < sattrs.length; b++ )
							{
								...
								switch ( pair[0].split(' ').join('').toLowerCase() )
								{
									case 'width':
										width = pair[1].split ( ' ' ).join ( '' );
										break;
									case 'height':
										height = pair[1].split ( ' ' ).join ( '' );
										break;
								}
							}
							attrs[z][1] = ed._lastClickedItem.getAttribute ( 'style' );
						}
