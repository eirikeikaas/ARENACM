...
class cDocument extends cPTemplate
{
	...
	function cDocument ( $opts = false, $encoding = 'utf-8' )
	{
		parent::cPTemplate ( $opts );
		$GLOBALS[ 'document' ] = &$this;
		if ( !$GLOBALS[ 'TopMenuExtension' ] )
	...
	**/
	...
	function searchContent ( $content, $limit = false )
	{
		...
		**/
		...
		if ( $rows = $db->fetchObjectRows ( "SELECT * FROM ( " . $query . " ) AS tquery", MYSQL_ASSOC ) )
		{
	...
	function render ( )
	{
		...
		// check settings =============================================
		$sets = new dbObject( 'Setting' );
		$sets->SettingType = 'contents';
		$top = '';
		if( $sets = $sets->find() )
		{
			foreach( $sets as $setting )
			{
				...
				{
					case 'footerline':
					case 'contents_footerline':
						$this->page->footerline = $setting->Value;
						break;
						
					case 'title':
					case 'contents_title':
						if( $setting->Value != '' ) $this->page->ExtraTitle = ' / ' . $setting->Value;
						break;
					
					case 'meta_keywords':
					case 'contents_meta_keywords':
						$top.= getLn() . '<meta name="keywords" content="'. $setting->Value .'" />';
						break;
						
					case 'meta_description':
					case 'contents_meta_description':
						$top.= getLn() . '<meta name="description" content="'. $setting->Value .'" />';
						break;
					
					case 'meta_copyright':
					case 'contents_meta_copyright':
						$top.= getLn() . '<meta name="copyright" content="'. $setting->Value .'" />';
						break;
					
					case 'meta_author':
					case 'contents_meta_author':
						$top.= getLn() . '<meta name="author" content="'. $setting->Value .'" />';
						break;		
