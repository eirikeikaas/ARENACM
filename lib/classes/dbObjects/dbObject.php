...
class dbObject
{
	...
	**/
	function dbObject ( $table=false, $database=false )
	{
	...
	**/
	function loadTable ()
	{
		if ( !is_object ( $this->_table ) || !$this->_table->table )
		{	
			$database =& $this->getDatabase ( );
			$this->_table	=& $database->getTable ( $this->_tableName );
			if ( !$this->_primaryKey ) 
	...
	**/
	...
	function &getDatabase ( )
	{
		...
		if ( $this->_table && $this->_table->database )
		{
			$tmp =& $this->_table->database;
			return $tmp;
		}
	...
	**/
	function exists ( $key=false )
	{
		...
		foreach ( $keys as $key ) if ( isset ( $this->{$key} ) )           // and populate it
		{
		...
		if ( count ( $keys ) == count ( $whereClause ) )                   // only do the query if we have enough values
		{
			...
			$row = $database->fetchSingle ( "
				...
				{$whereClause}
			" );
			//                                  WHERE `{$this->_primaryKey}` = ".$this->formatField( $this->_primaryKey, $this->{$this->_primaryKey} )."" );
	...
	**/
	function load ( $key=false )
	{
		...
		// Return false if we cannot get an array of field names (ie, the table is blank)
		if ( !is_array ( $this->_table->getFieldNames() ) )    return false;       
	...
	**/
	function find ( $customQuery = false )
	{
		...
		if ( !$query )
		{
			...
			$query .= ( $this->hasClause ( 'GROUP BY' ) ) ? ( ' GROUP BY ' . $this->_clauses[ 'GROUP BY' ] ) : '';
			$query .= ( $this->hasClause ( 'ORDER BY' ) ) ? ( ' ORDER BY ' . $this->_clauses[ 'ORDER BY' ] ) : '';
			$query .= ( $this->hasClause ( 'LIMIT' ) ) ? ( ' LIMIT ' . $this->_clauses[ 'LIMIT' ] ) : '';
				
		}
			
		// Fetch the results
		$rows = $database->fetchRows ( $query, MYSQL_ASSOC );
		
		$this->_lastQuery = $query;
	
		if ( function_exists ( 'clone' ) ) $php5 = true;
		else $php5 = false;
		...
		if ( $rows ) 
		{
			...
			foreach ( $rows as $row )
			{
				if ( strtolower ( $className ) == 'dbclassinstance' )
				{
					$obj = new dbClassInstance ( substr ( $this->_tableName, 5, strlen ( $this->_tableName ) - 5 ) );
				}
				else if ( strtolower ( $className ) != 'dbobject' )
				{
					$obj = new $className ( );
				}
				else
				...
				if ( $this->_dbOverride ) $obj->_dbOverride =& $this->_dbOverride;
	...
	**/
	...
	function findCount ( $customQuery = false )
	{
		...
		if ( !$query )
		{
			...
			if ( $this->_table->getFieldNames() ) foreach ( $this->_table->getFieldNames() as $field )
			{
				if ( isset ( $this->{$field} ) )      $queryWhere[] = "`{$field}` = ".$this->formatField ( $field, $this->{$field} );
			}
	...
	function searchTerms ( $searchTerms )
	{
		if ( !is_array( $searchTerms ) )
		{
			...
			//$searchTerms = preg_replace ( "/[=\/.-]/", "%20", $searchTerms );                                            // replace phrase bind chars with urlencoded space
	...
	**/
	function globalValueSet ( $key, &$new )
	{
		$GLOBALS[ '__GLOBALS__' ][ 'dbObject' ][ $key ] =& $new;
	}
	
	function &globalValueUnset ( $key )
	{
			...
			if ( substr ( $k, 0, 6 ) == 'Extra_' )
			{
				...
				if ( $obj->load ( $id ) )
				{
					switch ( $obj->Type )
					{
						case 'leadin':
						case 'text':
							$obj->$field = decodeArenaHTML ( cleanHTMLElement ( $v ) );
							break;
						default:
							$obj->$field = trim ( $v );
							break;
			...
			(
				(
					SELECT 
						...
						\"\" AS `DataMixed`, b.SortOrder, b.ID, b.Name, b.ContentGroup, b.IsVisible, b.AdminVisibility
					FROM 
					`ContentDataBig` b{$contentoptions1a} $extra1a
					WHERE 
					( b.ContentID='{$this->ID}' || ( b.IsGlobal{$contentoptions2a} ) )
					AND
				...
				(
					SELECT 
						...
						c.DataMixed, c.SortOrder, c.ID, c.Name, c.ContentGroup, c.IsVisible, c.AdminVisibility
					FROM 
					`ContentDataSmall` c{$contentoptions1b} $extra2a
					WHERE 
					( c.ContentID='{$this->ID}' || ( c.IsGlobal{$contentoptions2b} ) )
					AND
					c.ContentTable=\"{$this->_tableName}\" $exc
					$extra2b
				)
			) AS z
			GROUP BY z.Name ORDER BY z.SortOrder ASC, z.ID ASC
		";
		
		if ( $rows = $db->fetchObjectRows ( $query, MYSQL_ASSOC ) )
		{
			/**
	...
	**/
	function loadExtraFieldNames ( $options = false, $r = 0 )
	{
		if ( $r > 1 ) 
		{
			die ( 'Error with creating duplicate fields for editmode!!' );
		...
		if ( $this->_fieldsLoaded ) return true;
		$db =& $this->getDatabase ( );
