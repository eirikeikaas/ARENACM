...
{	
	...
	**/
	function dbUser ( $cookie_domain = false, $cookie_path = false )
	{
		global $application;
	
		// Ready cache
		if ( !$GLOBALS[ 'Cache' ][ 'SuperAdmin' ] )
	...
	**/
	function isSuperUser ( )
	{
		if ( $this->_dataSource == 'core' ) 
			return true;
		if ( $GLOBALS[ 'Cache']['SuperAdmin'][$this->ID] )
	...
	**/
	...
	function make_token ()
	{
		return md5( microtime() . date( 'r' ) . rand( 10000, 32000 ) );
	}
	...
	function onSaved ()
	{
		// Ensure login
		global $user;
		if ( $user->ID && ( $this->ID == $user->ID ) && @!$_REQUEST[ 'logout' ] )
		{
	...
	function loadGroups ()
	{
		if ( !$this->groups ) 
		{
			...
			if ( is_array ( $groups ) )
			{
				...
				{
					$rights = $group->Modules;
					if ( $group->Level == 'unlimited' )
					{
						$this->rights = 'all';
					}
	...
	**/
	...
	{
		$headers = "From: arena@blest.no\r\nReply-To: utvikling@blest.no\r\nContent-type: text/plain; charset=UTF-8\r\n";
		$recipient = $this->Email;
		
		// Generate info
		$fields = Array ( 'Address'=>'Adresse', 'Postcode'=>'Postnummer', 'City'=>'Poststed', 'Mobile'=>'Mobil', 'Telephone'=>'Telefon' );
		$addy = Array ( );
		foreach ( $fields as $k=>$v )
			if ( $this->$k ) $addy[] = "$v: {$this->$k}";
		$addy = count ( $addy ) ? ( "\n" . implode ( "\n", $addy ) ) : '';
		
		if ( $this->IsAdmin )
		{
			//mail_ ( $recipient, i18n ( 'Welcome new user!' ), $tpl->render ( ), $headers );
		}
		else
		{	
			mail_ ( $recipient, 'Velkommen til ' . SITE_TITLE, '
Velkommen til ' . SITE_TITLE . '!

Du er nå registrert som bruker. Vennligst logg inn med brukernavn og passord:

  Brukernavn: ' . $this->Username . '
  Passord: ' . ( $this->Password_unhash ? $this->Password_unhash : '(uendret)' ) . $addy . '
...
' . BASE_URL . '
	...
	function sendUpdateMail()
	{
		...
		else
		{
			mail_ ( 
				$recipient,
				'Oppdatert informasjon hos ' . SITE_TITLE, '
Hei ' . $this->Name . '!

Din konto er oppdatert med ny informasjon.
