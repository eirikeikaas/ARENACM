...
class dbImageFolder extends dbFolder
{
	...
	function dbImageFolder ( $id = false )
	{
		$this->loadTable ( );
		if ( $id ) $this->load ( $id );
	}
...
class dbImage extends dbObject 
{
	...
	**/
	function getImageUrl ( $width = false, $height = false, $mode = false, $effects = false, $bgcolor = false )
	{
		...
		{
			$path = BASE_URL . $this->getFolderPath ( );
			$path .= '/' . $this->Filename;
			
			if ( $info = @getimagesize ( BASE_DIR . '/' . $this->getFolderPath ( ) . '/' . $this->Filename ) )
			{
		...
		else
		{
			...
			{
				list ( $w, $h, ) = @getimagesize ( BASE_DIR . '/upload/images-master/' . $this->Filename );
				if ( $w && $h )
				{
					$this->Width = $w;
					$this->Height = $h;
			...
			**/
			...
			if ( !file_exists ( $this->getFolderPath ( ) . "/" . $this->Filename ) )
			{
				...
				switch ( $type )
				{
					...
					case IMAGETYPE_BMP:
						if ( function_exists ( "imagecreatefrombmp" ) )
							$image = imagecreatefrombmp ( BASE_DIR . '/' . $this->getFolderPath ( ) . "/" . $this->Filename );
						else return false;
						break;
				}
			}
	...
	function receiveUpload ( $file )
	{
		...
		if ( $file[ "tmp_name" ] )
		{
			...
			if ( !( move_uploaded_file ( $file[ "tmp_name" ], BASE_DIR . "/$folder/$filename" ) ) )
				return false;
	...
	{
		$folder = '';
		if ( $this->Folder && trim ( $this->Folder->DiskPath ) )
		{
			return $this->Folder->DiskPath;
		}
		return $folder = "upload/images-master";
	}
	...
	**/
	function renderCaptcha ( $width = false, $height = false, $background = 0xffffff, $foreground = 0x000000, $backimg = false, $text = false )
	{
		...
		$fn = BASE_DIR . '/upload/' . texttourl ( microtime ( ) ) . '_captcha.jpg';
		...
		return array ( '<img src="' . str_replace ( BASE_DIR . '/', BASE_URL, $fn ) . '" width="' . $width . '" height="' . $height . '" class="Captcha"/>', $out );
	}
