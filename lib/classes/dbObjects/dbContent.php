...
class dbContent extends dbObject
{
	...
	**/
	function dbContent ( $id = false )
	{
		global $Session;
		$this->loadTable ();
		$this->_language = $Session->CurrentLanguage ? $Session->CurrentLanguage : 1;
	...
	{
		parent::load ( $id );
		if ( $this->MainID != $this->ID && $this->ID > 0 )
			$this->_editmode = 1;
	...
	**/
	function getPath ( )
	{
		...
		if ( strlen ( $Session->Language->UrlActivator ) <= 0 )
			return $code . $oStr . "{$root}index.html";
		return $oStr . "{$root}index.html";
	}
	...
	**/
	function getByPath ( $route )
	{
		...
		if ( $_REQUEST[ 'editmode' ] ) $this->_editmode = 1;
		...
		if ( !$key )
		{
			...
			while ( count ( $paths ) )
			{
				...
				if ( substr ( trim ( $path ), 0, 9 ) != 'index.htm' ) 
				{
					...
					if ( $tmp = $db->fetchObjectRow ( $query, MYSQL_ASSOC ) )
					{
						$page = $tmp;
						$parent = $page->MainID;
					}
	...
	**/
	function renderExtraField ( $fieldObject, $options = false )
	{
		...
		switch ( $fieldObject->Type )
		{
			...
			case 'extension':
				...
				if ( !$this->_editmode || ARENAMODE == 'web' )
				{
					...
					if ( file_exists ( $target ) )
					{
						$content =& $this;
						include ( $target );
						return '<div id="' . $fieldObject->Name . '">' . $extension . '</div>';
					}
				}
				return '';
			...
			case 'whitespace':
				return "<div id=\"{$fieldObject->Name}\"></div>";	
	...
	**/
	...
	function RenderSelect ( $name, $id = '', $current = 0, $parent = 0, $r = '', $rgbc = Array ( 0, 0, 0 ), $rgbb = Array ( 230, 230, 230 ) )
	{
		global $LanguageID;
		$db =& parent::getDatabase ( );
		if ( $rows = $db->fetchObjectRows ( 'SELECT * FROM ContentElement WHERE Parent=' . (string)$parent . ' AND ID=MainID ' . ( $LanguageID > 0 ? ( 'AND `Language`=\'' . $LanguageID . '\'' ) : '' ) . 'ORDER BY SortOrder ASC, `Title` ASC' ) )
		{
			$ostr = "\n";
			if ( $r == '' )
			{
