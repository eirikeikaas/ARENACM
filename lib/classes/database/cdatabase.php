...
class cDatabaseTable
{
	...
	function cDatabaseTable ( $table=false, $database=false )
	{
	...
	{
		$fields = array ();
		if ( $fields_res = mysql_query( "DESCRIBE `{$this->table}`", $this->database->resource ) )
		{
	...
	**/
	...
	{ 
		...
		{
			$keys = Array();
			if ( $result = mysql_query ( 'SHOW KEYS FROM `' . $this->table . '`', $this->database->resource ) )
			{
...
class cDatabase
{
	...
	{         	 
		...
		if ( 
			$this->resource = mysql_connect ( 
				$this->hostname, $this->username, $this->password, true 
			) 
		)
		{
			if ( $this->db )
	...
	**/
	...
	function fetch2d ( $query, $mode = MYSQL_BOTH )
	{
		return $this->fetchRows ( $query, $mode );
	}

	function fetchRows ( $query, $mode = MYSQL_BOTH )
	{
		global $queries;
		$array = false; 
	...
	*/
	function fetchObjectRows ( $query, $mode = MYSQL_BOTH )
	{	
		if ( is_array ( $cached = $this->getCached ( $query . ' _OBJECTS_' ) ) )
			return $cached;
		if ( $qr = mysql_query ( $query, $this->resource ) )
		{
	...
	function fetchSingle ( $query, $mode = MYSQL_BOTH )
	{
		return $this->fetchRow ( $query, $mode );
	}
	
	// We're phasing out fetchSingle and fetch2d
	function fetchRow ( $query, $mode = MYSQL_BOTH )
	{
		global $queries;
	...
	function fetchObjectRow ( $query, $mode = MYSQL_BOTH )
	{		
		if ( is_array ( $cached = $this->getCached ( $query . ' _OBJECT_' ) ) )
			return $cached;
			
		if ( $rs = mysql_query ( $query, $this->resource ) )
		{
	...
	function query ( $query, $resource = false )
	{
		global $queries;
		...
		**/
		...
		if ( $resource )
		{
			if ( $result = mysql_query ( $query, $resource ) )
			{
	...
	function getId ( )
	{
		if ( $res = mysql_query( "SELECT LAST_INSERT_ID() AS ID", $this->resource ) )
		{
	...
	{
		if ( isset( $GLOBALS["__GLOBALS___{$this->db}"][$query] ) )
			return $GLOBALS["__GLOBALS___{$this->db}"][$query];
		else
			return false;
	}
...
------------------------------------------------------------------------- */
	...
	function loadTables ()
	{
	$tables = $this->fetch2d ( 'SHOW TABLES' );
	if ( sizeof ( $tables ) > 0 ) 
	{
		foreach ( $tables as $table ) 
		{
			$this->loadTable ( current ( $table ) );
		}
		return true;
	}
	}       
