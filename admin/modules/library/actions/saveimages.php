...
{
	$a = 0;
	while ( $_FILES[ 'image_' . ( string )$a ][ 'tmp_name' ] )
	{
		$img = new dbImage ( );
		$img->ImageFolder = $folder->ID;
		if ( $img->receiveUpload ( $_FILES[ 'image_' . ( string )$a ] ) )
		{
			if ( $_REQUEST[ 'imagename_' . ( string )$a ] )
			{
				$img->Title = $_REQUEST[ 'imagename_' . ( string )$a ];
			}
			else
			{
				$img->Title = $_FILES[ 'image_' . ( string )$a ][ 'name' ];
			}
			$img->save ( );
		}
		$a++;
	}	
	$success = 'Lagret ' . $a . ' bilder.';
}
else $success = 'Feilet..';
