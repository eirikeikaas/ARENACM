...
function deleteLibraryFile( fileID )
{
	if( document.getElementById( "FileEditContainer" ) )
	{
		...
		var confirmtxt = "";
		confirmtxt+= "<h1>Slett fil \""+ document.getElementById( "fileTitle" ).value +"\"</h1>";
		confirmtxt+= "<div class='Container'>";
		confirmtxt+= "	<h2 class='question'>Er du sikker at du vil slette denne filen ?</h2>";
		confirmtxt+= ' <div class="SpacerSmall"></div><div class="SpacerSmall"></div>';
		confirmtxt+= '	<button class="Small" onclick="doDeleteLibraryFile(' + fileID  + ');"><img src="admin/gfx/icons/page_delete.png" /> Slett</button>';
...
function doDeleteLibraryFile( fileID )
{
	...
	document.deljax.openUrl ( modulepath + '&action=deletefile&fileID=' + fileID, 'GET', true );
...
function submitFileUpload()
{
	...
	{
		var emsg = "Du har ikke valgt en gyldig fil ennå."
...
function checkFileUpload ()
{
	if ( document.uploadForm.uploadFile && document.uploadForm.uploadFile.value )
	{
		...
		if ( fileName.toLowerCase().match( /\.(jpg|jpeg|png|gif)$/ )  )
		{
			...
			var emsg = "Du har valgt et bildefil. Kanskje du burde bruke bildeopplastningsverktøyet."
...
function texteditNormal ( )
{
	...
	document.getElementById ( 'tekstFullscreen' ).innerHTML = '<img src="admin/gfx/icons/arrow_out.png"/> Fullskjerm';
