...
if ( intval ( $user->ID ) > 0 )
{
	...
	' ) ) )
	{
		if ( $row = $db->fetchObjectRow ( 'SELECT * FROM Users WHERE IsTemplate = 1' ) )
		{
			...
			{
				$objs = new dbObject ( "ContentData{$k}" );
				$objs->addClause ( 'WHERE', 'ContentID=\'' . $row->ID . '\' AND ContentTable="Users"' );
				$objs->addClause ( 'ORDER BY', 'SortOrder ASC, ID ASC' );
				if ( $objs = $objs->find ( ) )
				{
					// Go through all the content data for the template user
					foreach ( $objs as $obj )
					{
						// If there is no field with this name from before for this user
						if ( !$db->fetchObjectRow ( '
							SELECT * FROM ContentData' . $k . ' 
							WHERE 
							ContentTable = "Users" AND ContentID = \'' . $user->ID . '\' AND Name="' . $obj->Name . '"
						' ) )
						{
							// Create for this user
							unset ( $obj->ID );
							$obj->ContentID = $user->ID;
							$obj->SortOrder = $i;
							$obj->save ( );
							$i++;
						}
